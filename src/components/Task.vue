<template>
    <div @contextmenu.prevent="setTooltip" :class="[task.isDone ? 'project__done' : '', 'project__task']">
      <p @click="editMode(true)" v-if="!editTask.isMode" class="project__task-description">{{ task.description }}</p>
      ...
      <img 
        ...
        @click="setDone(false)" 
        v-if="task.isDone && !editTask.isMode" 
  ...
  methods: {
    ...
    async saveText() {
      if (this.editTask.value.length > 0) {
        await this.$store.dispatch("saveDescription", { value: this.editTask.value, taskID: this.task.id })
        this.editMode(false)
